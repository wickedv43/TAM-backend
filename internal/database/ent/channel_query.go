...
func (_q *ChannelQuery) QueryCustomerChannels() *CustomerChannelQuery {
...
func (_q *ChannelQuery) QueryDeals() *DealQuery {
...
func (_q *ChannelQuery) WithCustomerChannels(opts ...func(*CustomerChannelQuery)) *ChannelQuery {
	query := (&CustomerChannelClient{config: _q.config}).Query()
...
func (_q *ChannelQuery) WithDeals(opts ...func(*DealQuery)) *ChannelQuery {
	query := (&DealClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ChannelQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Channel, error) {
	...
	if query := _q.withCustomerChannels; query != nil {
		if err := _q.loadCustomerChannels(ctx, query, nodes,
			func(n *Channel) { n.Edges.CustomerChannels = []*CustomerChannel{} },
			func(n *Channel, e *CustomerChannel) { n.Edges.CustomerChannels = append(n.Edges.CustomerChannels, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withDeals; query != nil {
		if err := _q.loadDeals(ctx, query, nodes,
			func(n *Channel) { n.Edges.Deals = []*Deal{} },
			func(n *Channel, e *Deal) { n.Edges.Deals = append(n.Edges.Deals, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ChannelQuery) loadCustomerChannels(ctx context.Context, query *CustomerChannelQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *CustomerChannel)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(customerchannel.FieldChannelID)
	}
	query.Where(predicate.CustomerChannel(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(channel.CustomerChannelsColumn), fks...))
	}))
...
func (_q *ChannelQuery) loadDeals(ctx context.Context, query *DealQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *Deal)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Channel)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(deal.FieldChannelID)
	}
	query.Where(predicate.Deal(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(channel.DealsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
